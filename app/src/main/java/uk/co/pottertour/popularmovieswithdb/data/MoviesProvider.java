...
public class MoviesProvider extends ContentProvider {
     ...
     * reason not to.
     *
     * @return A UriMatcher that correctly matches the constants for CODE_MOVIES and CODE_WEATHER_WITH_DATE
     */
    public static UriMatcher buildUriMatcher() {
         ...
         * they aren't going to change. In Sunshine, we use CODE_MOVIES or CODE_WEATHER_WITH_DATE.
         */

        /* This URI is content://com.example.android.sunshine/movies/ */
    ...
    public Cursor query(@NonNull Uri uri, String[] projection, String selection,
        ...
        switch (sUriMatcher.match(uri)) {
             ...
             */
            case CODE_MOVIES: {
                Log.wtf(TAG, "CODE_MOVIES: opening a readable database, making a query");
                Log.wtf(TAG, "selection_col: " + selection);
                Log.wtf(TAG, "selection_vals: " + selectionArgs);

                try{
                    Log.v(TAG, "Trying to find row id from: " + String.valueOf(uri));
                    Long rowId = ContentUris.parseId(uri);
                    Log.v(TAG, "Row _ID found: " + rowId);
        ...
        Log.wtf(TAG, "Exiting Movies Provider with cursor rows = " + cursor.getCount());
        return cursor;
    }
     ...
     * override it as MoviesProvider extends ContentProvider and getType is an abstract method in
    ...
    /**
     * In Sunshine, we aren't going to do anything with this method. However, we are required to
     * override it as MoviesProvider extends ContentProvider and insert is an abstract method in
     * ContentProvider. Rather than the single insert method, we are only going to implement
     * {@link MoviesProvider#bulkInsert}.
     *
    ...
    @Override
    public int update(@NonNull Uri uri, ContentValues newValues, String selection, String[] selectionArgs) {
        ...
        switch (sUriMatcher.match(uri)) {
            ...
            case CODE_TRAILERS:
                ...
                if (rowsUpdated > 0) {
                    Log.wtf(TAG, "updated " + rowsUpdated + " in db, notifying change, yay!");
